...
-- Listage de la structure de table forum_erwin. comment_post
...
-- Listage des données de la table forum_erwin.comment_post : ~168 rows (environ)
INSERT INTO `comment_post` (`id_comment`, `text`, `commentDate`, `post_id`, `user_id`) VALUES
...
-- Listage de la structure de table forum_erwin. comment_publication
CREATE TABLE IF NOT EXISTS `comment_publication` (
  `id_comment` int NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `FK_commentpublication_publication` FOREIGN KEY (`publication_id`) REFERENCES `publication` (`id_publication`) ON DELETE CASCADE,
  CONSTRAINT `FK_commentpublication_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id_user`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Listage des données de la table forum_erwin.comment_publication : ~0 rows (environ)
...
-- Listage de la structure de table forum_erwin. like_post
...
-- Listage des données de la table forum_erwin.like_post : ~0 rows (environ)

-- Listage de la structure de table forum_erwin. like_publication
CREATE TABLE IF NOT EXISTS `like_publication` (
  `publication_id` int NOT NULL,
  `user_id` int NOT NULL,
  KEY `FK__publication` (`publication_id`),
  KEY `FK__user` (`user_id`),
  CONSTRAINT `FK__publication` FOREIGN KEY (`publication_id`) REFERENCES `publication` (`id_publication`) ON DELETE CASCADE,
  CONSTRAINT `FK__user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id_user`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Listage des données de la table forum_erwin.like_publication : ~0 rows (environ)
...
INSERT INTO `topic` (`id_topic`, `title`, `user_id`, `category_id`, `creationDate`, `views`, `closed`) VALUES
	(100, 'Quel est le meilleur joueur de football de tous les temps ?', 1, 34, '2025-01-24 16:19:36', 7, 0),
...
-- Listage de la structure de table forum_erwin. under_comment_post
CREATE TABLE IF NOT EXISTS `under_comment_post` (
  `text` varchar(255) NOT NULL,
  `commentDate` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `user_id` int DEFAULT NULL,
  ...
  CONSTRAINT `FK_undercommentpost_commentpost` FOREIGN KEY (`comment_id`) REFERENCES `comment_post` (`id_comment`) ON DELETE CASCADE,
  CONSTRAINT `FK_undercommentpost_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id_user`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Listage des données de la table forum_erwin.under_comment_post : ~45 rows (environ)
INSERT INTO `under_comment_post` (`text`, `commentDate`, `user_id`, `comment_id`) VALUES
...
INSERT INTO `user` (`id_user`, `nickName`, `password`, `dateInscription`, `avatar`, `email`, `role`, `isBanned`) VALUES
	(1, 'josh', 'gfsgfgfsg', '2024-12-02 16:52:01', 'default-avatar.webp', 'josh@exemple.com', 'SUPER_ADMIN', 'actif'),
